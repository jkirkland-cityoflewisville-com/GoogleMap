  ...
  MapOverlay.prototype.refresh = function () {	  
	...
	//Clear overlay when dragging is done
		...
		if (!this.dragging && this.overlay_){
		  //Do on Zoom		  
		  //this.div_.style.transform = "scale(2)";
		  console.log("removing overlay - zoom")
    ...
    this.mapService_.exportMap(params, function (json) {
      ...
      if (json.href) {
        ...
        //  me.overlay_ = null;
        //}
		//console.log("creating new ImageOverlay")
		//x(me);
		...
		//toDataUrl(json.href, function(base64Img) {
		//	me.base64Img = base64Img;
		//});
      }
      /**
...
x = function(overlay_){
	console.log("x")
	console.log(overlay_)
	if(overlay_){
		var div = document.createElement('DIV');
		div.style.border = "none";
		div.style.borderWidth = "0px";
		div.style.position = "absolute";
		var s = overlay_.map_.getDiv();
		div.style.width = s.offsetWidth + 'px';
		div.style.height =  s.offsetHeight + 'px';
		div.style.transform = "scale(2)";
		//div.style.animation = "bounceIn 0.5s"
		div.style.backgroundImage = 'url(' + overlay_.base64Img + ')';
		div.style.zIndex = overlay_.__ZINDEX;
		
		overlay_.div_ = div;
		
		// We add an overlay to a map via one of the map's panes.
		// We'll add this overlay to the overlayImage pane.
		var panes = overlay_.getPanes();
		setNodeOpacity_(div, overlay_.op_);
   ...
   * @param {Object} map
   */
  function ImageOverlay(bounds, url, map, op, __ZINDEX) {
    this.bounds_ = bounds;
    this.url_ = url;
    this.map_ = map;
    this.div_ = null;
